...
func SignUp(ctx *gin.Context){
	...
	httputil.WriteJson(ctx,true,200,res)
	
}
func Login(ctx *gin.Context){
	
	res, err := LoginUser(ctx)
	if err != nil {
		httputil.WriteJson(
			ctx,
			false,
...
func RegisterUser(ctx *gin.Context) (*models.JsonResponse, error) {
//	var JsonResponse models.JsonResponse
	...
	filter := bson.M{"_id": insertedId}
		...
		err = patientCollection.FindOne(ctx, filter).Decode(&createdUser)
		if err != nil {
			return nil, err
		}
		createdUser.ID = insertedId
...
func LoginUser(ctx *gin.Context) (*models.JsonResponse, error) {
	...
	//}
	token, err := GenerateToken(userdetail)
...
func emailExists(email string, dataB *mongo.Database) (bool, error) {
	user := models.User{}
	patientCollection := dataB.Collection("user")
...
func GenerateTokenVia(userDetails models.UserDetails) (models.Tokens, error) {
